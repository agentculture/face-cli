...
face-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. face-cli
...
#   - consumer-identifying prose: `guildmaster` → `face-cli` (NOT
...
If a re-sync would lose a face-cli adaptation, lift the change
...
#     → `which face-cli vendors from guildmaster`
