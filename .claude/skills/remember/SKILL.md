...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope face-cli
...
scope:** the default personal scope is **private** (`--scope face-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `face-cli`). Private records are served only to a recall in the same
