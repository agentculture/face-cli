...
- **Re-vendored the `ask-colleague` skill from colleague (now 1.7.0, up from the
  ...
  consumer-identifying Provenance clause (`face-cli vendors from
...
- **Vendor the `outsource` skill** (`.claude/skills/outsource/`) from
  ...
  *different* engine/mind: `explore` (read-only investigation), `review` (a
  ...
  [#8](https://github.com/agentculture/face-cli/issues/8).
...
- Expanded the clone-and-rename instructions in `CLAUDE.md`: added `README.md` to
  ...
  `face_cli/cli/__init__.py`) rather than renaming by hand.
...
- SonarCloud now reports code coverage — added `relative_files = true` to
  ...
  `sonar.sources=face_cli` (absolute / `.venv` paths were dropped
...
- **CI gates on the SonarCloud quality gate**
  ([issue #3](https://github.com/agentculture/face-cli/issues/3)) —
...
- **Onboarded into the AgentCulture mesh** ([issue #1](https://github.com/agentculture/face-cli/issues/1)).
...
- **Mesh identity**: `culture.yaml` (`suffix: face-cli`,
