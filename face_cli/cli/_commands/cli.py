"""``face-cli cli`` — noun grouping CLI-surface introspection.
...
from face_cli.cli._commands.overview import cli_sections, emit_overview
...
def _no_verb(args: argparse.Namespace) -> int:
    # `face-cli cli` with no sub-verb prints the noun's overview.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "cli",
        help="CLI-surface introspection (see 'face-cli cli overview').",
    )
    ...
    ov = noun_sub.add_parser("overview", help="Describe the face-cli CLI surface.")
