"""Smoke tests for the face-cli CLI entry point and its verbs."""
...
from face_cli.explain import known_paths
...
def test_explain_root(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain"])
    assert rc == 0
    assert "# face-cli" in capsys.readouterr().out


def test_explain_self(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain", "face-cli"])
    assert rc == 0
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "face-cli whoami" in payload["markdown"]
